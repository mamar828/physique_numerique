...
with open(f"projet/data/neural_networks/{fitter_class_name}/info.csv", "a") as f:
    if f.tell() == 0:   # if the file is empty, write the header
        f.write(f"file,n_epochs,batch_size,n_train_samples,n_valid_samples,n_test_samples,R^2,MSE,training_time,date\n")

    f.write(f"{SPEC_FILE},{N_EPOCHS},{BATCH_SIZE},{len(train_set)},{len(valid_set)},{len(test_set)},{r2:.6f},{mse:.6f},"
